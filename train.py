...
import argparse
from models.GFNet_final import *
...
parser.add_argument('--device', default="2", type=str, help='which gpu use')
...
parser.add_argument("--root_dir", type=str, default='/sdc/cjh/deep_super/nyu_data', help="root dir of dataset")
...
net = GFNet(num_feats=opt.num_feats, kernel_size=3, scale=opt.scale).cuda()
# 计算模型参数数量
...
for epoch in range(max_epoch):
    ...
    for idx, data in enumerate(t):
        ...
        optimizer.zero_grad()
        guidance, lr, gt, coord, cell = data['guidance'].cuda(), data['lr'].cuda(), data['gt'].cuda(), data['hr_coord'].cuda(), \
                                        data['cell'].cuda
        out, out_grad = net((guidance, lr, coord, cell))
    ...
    if ((epoch) % 2 == 0) and (epoch < 30):
        with torch.no_grad():
            ...
            for idx, data in enumerate(t):
                if dataset_name == 'nyu_data':
                    guidance, lr, gt, coord, cell = data['guidance'].cuda(), data['lr'].cuda(), data['gt'].cuda(), data['hr_coord'].cuda(), data['cell'].cuda()
                    out, out_grad = net((guidance, lr, coord, cell))
        ...
        with torch.no_grad():
            ...
            for idx, data in enumerate(t):
                if dataset_name == 'nyu_data':
                    guidance, lr, gt, coord, cell = data['guidance'].cuda(), data['lr'].cuda(), data['gt'].cuda(), data[
                        'hr_coord'].cuda(), data['cell'].cuda()
                    out, out_grad = net((guidance, lr, coord, cell))
                    minmax = test_minmax[:, idx]
                    minmax = torch.from_numpy(minmax).cuda()
                    rmse[idx] = calc_rmse(gt[0, 0], out[0, 0], minmax)
