from models.common_up_down import *
import torch
...
class GFNet(nn.Module):
    def __init__(self, num_feats, kernel_size, scale):
        ...
        # self.upsample = Upsampler(default_conv, scale, 3*num_feats)
    ...
    def forward(self, x):
        ...
        dp_in = self.act(self.conv_dp1(depth))  # 深度图卷积，通道变多，好拼接
        dp1 = self.dp_rg1(dp_in)    # 残差组

        cat10 = torch.cat([dp1, grad_d4], dim=1)
        dp1_ = self.c_grad(cat10)

        rgb1 = self.act(self.conv_rgb1(image)) # 对rgb卷积
        rgb2 = self.rgb_rb2(rgb1)   # 对rgb残差处理

        ca1_in, r1 = self.bridge1(dp1_, rgb2)
        #######
        # dp2 = self.dp_rg2(torch.cat([dp1, ca1_in], 1))
    ...
    # ]
    #
    # self.last_conv2 = nn.Sequential(*last_conv2)
    #
    # self.imnet = nn.Sequential(
    #     nn.Conv2d(in_channels=num_feats,
    #               out_channels=num_feats, kernel_size=3, padding=1),
    #     nn.LeakyReLU(),
    #     nn.Conv2d(in_channels=num_feats, out_channels=1, kernel_size=3, stride=1, padding=1)
    # )
