...
class SDB(nn.Module):
    ...
    def forward(self, dp, rgb):
        ...
        dp_amp = torch.abs(dp)  # 幅度
        dp_pha = torch.angle(dp)  # 相位
...
class SFIB(nn.Module):
    def __init__(self, channels, rgb_channels, scale):
        ...
        # self.upBlock = Upsampler(default_conv, scale, channels)
        # self.downBlock = Downsampler(default_conv, scale, channels)
    ...
    def forward(self, dp, rgb):  # , i
        ...
        #  如果这句话不要会怎么样
        cha_res = self.cha_att(self.contrast(cat_f) + self.avgpool(cat_f)) * cat_f  # （通道维度处理+注意力）* 本身，网络图并没画出来
...
class GFE(nn.Module):
    def __init__(self, n_feats, scale):
        ...
        # self.upBlock = Upsampler(default_conv, scale, 1)
        # self.downBlock = Downsampler(default_conv, scale, n_feats)
    ...
    def forward(self, depth, rgb):
        depth = self.upBlock(depth)   # 深度图上采样

        grad_rgb = self.grad_rgb(rgb)  # 求rgb梯度, 每个通道分开求
        grad_d = self.grad_d(depth)  # 求深度图梯度

        rgb1 = self.c_rgb(grad_rgb)  # 卷积
        d1 = self.c_d(grad_d)  # 卷积

        rgb2 = self.rb_rgbd(rgb1)  # 残差块
        d2 = self.rg_d(d1)  # 残差组

        cat1 = torch.cat([rgb2, d2], dim=1)

        inn1 = self.fuse_process(cat1)  # 残差组

        d3 = d1 + self.CA(inn1)

        grad_d2 = self.c_fuse(d3)  # 卷积

        out_re = self.re_g(grad_d2)  # 这个是我们生成的残差图

        # return out_re, grad_d2
        # ###########
        d4 = self.d1(depth)
